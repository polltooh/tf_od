    ...
    for annot in annotation:
      example = tf.train.Example(
          features=tf.train.Features(
              feature={
                  "image_name":
                      encode_bytes(annot["image_name"]),
                  "mask_name":
                      encode_bytes(annot["mask_name"]),
                  "bboxes":
                      encode_float(
                          np.array(annot["bboxes"]).reshape(-1).tolist()),
                  "labels":
                      encode_int64(annot["labels"])
              }))
...
if __name__ == "__main__":
  cam_num = "253"
  annotation = json.load(open("../file_list/{}.json".format(cam_num), "r"))
  random.shuffle(annotation)
  train_ratio = 0.8
  train_len = int(len(annotation) * train_ratio)
  train_filename = "../data/{}_train.tf".format(cam_num)
  val_filename = "../data/{}_val.tf".format(cam_num)
